...
export async function GET(request: Request) {
  ...
  try {
    ...
    const week = [...league.weeks]
      .reverse()
      .find((candidate) =>
        candidate.payout === 0 &&
        candidate.picks.length === 5 &&
        candidate.picks.every((pick) => pick.text && pick.result === 'win') &&
        !requested.has(candidate.date),
      );
    return week
      ? json({ shouldNotify: true, week: { date: week.date } })
      : json({ shouldNotify: false });
  } catch {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return json({ error: 'Could not mark the payout request as sent.' }, 400);
  }
}
